        ...
        <TabPane tab="热门职位" key="1">
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
            {(MockRecommendJobs as JobPosition[]).map(jobInfo => (
              <JobPositionCard key={jobInfo.id} jobInfo={jobInfo} onClick={() => window.open('/job/' + jobInfo.id, '_blank')} />
            ))}
          </div>
        </TabPane>
      </Tabs>
        ...
        <TabPane tab="热门公司" key="1">
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-3">
            {(MockRecommendCompanys as Company[]).map(companyInfo => (
              <CompanyCard key={companyInfo.name} companyInfo={companyInfo} onClick={() => window.open('/company/' + companyInfo.name, '_blank')} />
            ))}
          </div>
        </TabPane>
      </Tabs>
